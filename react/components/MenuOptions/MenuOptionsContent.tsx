...
const MenuOptionsContent = (props: MenuOptionsContentProps): JSX.Element => {
  ...
  return (
    <Popover onOutsideClick={onClose}>
      <div className="bg-base shadow-3" onMouseEnter={onClose}>
        {map(
          item => (
            <div
              key={item.title}
              className={classNames(
                'w-100 bb b--muted-4 pv4 ph8 c-muted-1 flex justify-center pointer',
                {
                  'bg-disabled': item.disabled,
                }
              )}
              onMouseEnter={() => !item.disabled && item.onClick()}
            >
              {item.title}
            </div>
          ),
